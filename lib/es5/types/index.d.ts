export interface IWeb3NeyraAuth {
    ...
    signMessage: (setError: (error: string) => void, provider: any) => Promise<SignedMessage>;
    handlers: string[];
    callback: Callback;
}
type CallbackTypeNames = 'onSuccess' | 'onError' | 'handleSignUp' | 'addNotification';
type Callback = ({ type, params, }: {
